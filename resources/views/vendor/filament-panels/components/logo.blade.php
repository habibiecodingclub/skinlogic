@php
    ...
    $BrandColor = 'color: #D7CC8A';
    $brandClass = 'text-xl font-semibold leading-5 tracking-tight  text-white';
...
@capture($content, $logo, $isDarkMode = false)
    ...
    @elseif (filled($logo))
        <img alt="{{ __('filament-panels::layout.logo.alt', ['name' => $brandName]) }}" src="{{ $logo }}"
            {{ $attributes->class([$getLogoClasses($isDarkMode)])->style([$logoStyles]) }} />
        <h3 {{ $attributes->class([$brandClass])->style([$BrandColor]) }}>{{ $brandName }}</h3>
    @else
        <div
            {{ $attributes->class([
                $getLogoClasses($isDarkMode),
                'text-xl font-bold leading-5 tracking-tight  text-[#D7CC8A] ',
            ]) }}>
            {{ $brandName }}
        </div>
    @endif
