    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-20">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-3 group">
                ...
                <div>
                    <h1 class="text-2xl font-bold fo text-white tracking-wide font-poppins">SkinLogic</h1>
            ...
            <nav class="hidden lg:flex items-center gap-8">
                ...
                <button @click="reservationModal = true"
                        class="flex items-center gap-2 bg-white text-[#001a4d] px-6 py-2.5 rounded-full font-semibold font-poppins hover:bg-blue-50 hover:scale-105 transition-all duration-300 shadow-md font-poppins">
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            <div x-show="reservationModal"
                ...
                <div class="text-center mb-6">
                    <h3 class="text-2xl font-bold font-poppins text-[#001a4d]">
                        Reservasi: <span class="text-[#c5a365]">Umum</span>
                    </h3>
                    <p class="text-xs font-poppins text-gray-400 mt-1">Pilih metode pemesanan yang Anda inginkan.</p>
                </div>

                {{-- Tabs --}}
                <div class="flex border-b border-gray-200 mb-6">
                    <button @click="activeTab = 'whatsapp'"
                            :class="activeTab === 'whatsapp' ? 'border-[#c5a365] font-poppins text-slate-900' : 'border-transparent text-gray-400 hover:text-gray-600'"
                            class="flex-1 pb-3 text-sm font-semibold font-poppins border-b-2 transition-colors duration-300 flex items-center justify-center gap-2">
                    ...
                    <button @click="activeTab = 'website'"
                            :class="activeTab === 'website' ? 'border-[#c5a365] font-poppins text-slate-900' : 'border-transparent text-gray-400 hover:text-gray-600'"
                            class="flex-1 pb-3 text-sm font-semibold font-poppins border-b-2 transition-colors duration-300 flex items-center justify-center gap-2">
                ...
                <div x-show="activeTab === 'whatsapp'" x-transition:enter="transition ease-out duration-200 opacity-0" x-transition:enter-end="opacity-100">
                    <div class="space-y-4">
                        <div>
                            <label class="block text-xs font-bold font-poppins text-gray-700 mb-1">Nama Lengkap</label>
                            <input type="text" x-model="waName" placeholder="Contoh: Budi Santoso" class="w-full px-4 py-3 rounded-lg border border-gray-200 text-sm font-poppins focus:border-[#001a4d] focus:ring-1 focus:ring-[#001a4d] outline-none transition">
                        </div>
                        <div>
                            <label class="block text-xs font-bold font-poppins text-gray-700 mb-1">Tanggal Rencana</label>
                            <input type="date" x-model="waDate" class="w-full px-4 py-3 rounded-lg border border-gray-200 text-sm font-poppins text-gray-500 focus:border-[#001a4d] focus:ring-1 focus:ring-[#001a4d] outline-none transition">
                        </div>

                        {{-- Tombol Chat Admin (Hijau) --}}
                        <a :href="'https://wa.me/628123456789?text=Halo Admin, saya ' + waName + ' ingin reservasi untuk tanggal ' + waDate"
                           target="_blank"
                           class="block w-full bg-[#25d366] hover:bg-[#20bd5a] text-white text-center font-poppins font-bold py-3 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 mt-6 flex items-center justify-center gap-2">
                ...
                <div x-show="activeTab === 'website'" x-transition:enter="transition ease-out duration-200 opacity-0" x-transition:enter-end="opacity-100">
                    <form action="#" method="POST" class="space-y-3">
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-[10px] font-bold font-poppins text-gray-700 mb-1">Nama Lengkap</label>
                                <input type="text" class="w-full px-3 py-2 rounded-md border border-gray-200 text-xs font-poppins focus:border-[#001a4d] outline-none">
                            </div>
                            <div>
                                <label class="block text-[10px] font-bold font-poppins text-gray-700 mb-1">No. WhatsApp</label>
                                <input type="text" class="w-full px-3 py-2 rounded-md border border-gray-200 font-poppins text-xs focus:border-[#001a4d] outline-none">
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-[10px] font-bold font-poppins text-gray-700 mb-1">Tanggal</label>
                                <input type="date" class="w-full px-3 py-2 rounded-md border border-gray-200 text-xs font-poppins text-gray-500 focus:border-[#001a4d] outline-none">
                            </div>
                            <div>
                                <label class="block text-[10px] font-poppins font-bold text-gray-700 mb-1">Jam</label>
                                <select class="w-full px-3 py-2 rounded-md border border-gray-200 font-poppins text-xs text-gray-500 focus:border-[#001a4d] outline-none">
                                    <option>10:00</option>
                                    <option>13:00</option>
                                    <option>16:00</option>
                                </select>
                        ...
                        {{-- Metode Pembayaran --}}
                        <div>
                            <label class="block text-[10px] font-bold font-poppins text-gray-700 mb-2">Metode Pembayaran</label>
                            <div class="grid grid-cols-3 gap-2">
                                <button type="button" @click="paymentMethod = 'qris'"
                                        :class="paymentMethod === 'qris' ? 'border-[#001a4d] bg-blue-50  text-[#001a4d]' : 'border-gray-200 font-poppins text-gray-500'"
